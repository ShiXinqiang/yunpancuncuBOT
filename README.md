...
- 管理员全员广播，保留 Telegram 文字格式及媒体说明文字
- PostgreSQL 持久化存储
...
1. 管理员在机器人私聊发送 `/ads`，选择“📣 全员广播”。
2. 直接发送已排好格式的消息，也可发送图片、视频或文件并添加说明文字。
3. 检查机器人生成的预览，点击“确认发送”。
4. 机器人会显示发送进度以及成功、不可达和失败数量。

广播使用 Telegram 的消息复制能力，因此会保留粗体、斜体、链接等文字实体和媒体说明文字。用户名单保存在 `bot_users` 表中；升级时会自动补录历史上传用户，之后所有与机器人发生交互的用户都会自动记录。无法接收消息的用户会被标记，避免后续广播反复发送失败。
